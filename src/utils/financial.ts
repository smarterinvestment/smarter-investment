...
export const calculateFinancialSummary = (
  ...
  // Calculate savings rate with sensible limits
  // If no income, savings rate is 0
  // If spending more than earning, cap at -100%
  ...
  if (totalIncome > 0) {
    ...
    // No income but has expenses = -100%
    savingsRate = -100;
  }
...
export const getRequiredMonthlySaving = (goal: Goal): number | null => {
  ...
  const days = getDaysUntilDeadline(goal);
  
  if (!days || days <= 0) return remaining > 0 ? remaining : null;
  const result = remaining / (days / 30);
