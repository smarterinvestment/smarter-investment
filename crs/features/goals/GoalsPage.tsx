...
export const GoalsPage: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    return { active: active.length, completed: completed.length, totalTarget, totalSaved, progress: totalTarget > 0 ? (totalSaved / totalTarget) * 100 : 0 };
  }, [goals]);
                ...
                <Card className={cn('relative overflow-hidden', goal.isCompleted && 'border-success-500/50')}>
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="relative h-3 bg-white/10 rounded-full overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5 }} className="absolute h-full rounded-full" style={{ backgroundColor: goal.color }} />
                    </div>
                  </div>

                  {/* Amounts */}
                  <div className="flex justify-between text-sm mb-4">
                    <div><p className="text-white/50">Ahorrado</p><p className="font-semibold text-white">{formatCurrency(goal.currentAmount, currency)}</p></div>
                    <div className="text-right"><p className="text-white/50">Objetivo</p><p className="font-semibold text-white">{formatCurrency(goal.targetAmount, currency)}</p></div>
                  </div>
